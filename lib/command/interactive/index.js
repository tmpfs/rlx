...
module.exports = function interactive(info, req, next) {
  ...
  function assign(err, req, next) {
    ...
    console.log('parse complete %s', req.errors.length);

    if(!err && (!req.errors || req.errors.length === 0)
      && req.result.unparsed.length) {
      return next(this.wrap(
  ...
  function run() {
    ...
    ps.on('value', function(val, item) {
      ...
      scope.parse(val, function(err, req, parameters, e) {
        ...
        //console.log('parse complete %s', ps.current.data.location);
        //delete ps.current.data.location;

        //console.log('parse complete %s', err);

        if(err && err !== true && err.bail !== true) {
        ...
        if(req && !req.command) {
          return assign.call(scope, err, req, function(err) {
            if(err && err !== true && err.bail !== true) {
              // little dance to ensure error parameters and
              // default error argument lookup are respected
              return req.error(err, req, function(err, args) {
