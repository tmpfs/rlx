...
function print(doc, req, next) {
  ...
  // printing has been suppressed foe this execution
  if(conf.suppress) {
    conf.suppress = false;
    return next();
  }
  ...
  function run(err, req, next, contents) {
    //console.log('run print logic %s', err);
    if(err) return req.error(err, req, next);
    var isStream = (contents instanceof Readable);
    //console.log('run isStream %s', isStream);
    if(isStream) {
      ...
      //console.log('PRINT CALLING NEXT');
      next();
    }
  }
