...
function auth(info, req, err, cb) {
  ...
  if(authRequired && hasCredentials) {
    ...
    //console.log('asking ');
    return ask.call(scope, info, req, function(err, res) {
...
function ask(info, req, cb) {
  var opts = {id: prompt.AUTH, defaults: {name: this.username}};
  prompt.call(this, opts, function(err, result) {
