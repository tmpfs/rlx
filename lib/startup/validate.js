...
function server(req) {
  ...
  var cmd = unparsed[0], i, j, names, cmds;

  // quick fix
  if(unparsed.length === 1 && cmd === 'docs') return true;

  // let the empty logic handle this
  if(!cmd) return true;
  for(i = 0;i < exceptions.length;i++) {
    if(!Array.isArray(exceptions[i])) {
      names = exceptions[i].names();
      if(~names.indexOf(cmd)) {
        return true;
      }
    }else{
      cmds = unparsed.slice(0, exceptions[i].length);
      for(j = 0;j < exceptions[i].length;j++) {
        if(cmds[j] !== exceptions[i][j]) {
          return false;
        }
      }
      return true;
  ...
  async.concat(this.searchPath, function(pth, callback) {
    ...
    fs.stat(pth, function(err, stats) {
      if(err || !stats || stats && !stats.isDirectory()) {
        return callback(scope.wrap(errors.EDIRECTORY_REQUIRED, [pth]));
      }
      callback(null, pth);
    })
  }, function(err, result) {
