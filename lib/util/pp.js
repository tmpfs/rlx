...
function format(prefix, msg, parameters, record, multiline) {
  ...
  //if(typeof multiline === undefined) {
    //mutiline = hasNewline(msg, parameters);
  //}
  var fill = '';
  var id = '' + record.id;
  if(id.length < 6) {
...
function toString(opts) {
  ...
  if(opts.pad && hasNewline(opts.msg, opts.parameters)) {
    ...
    lines.forEach(function(line) {
      ...
      line = util.format.apply(util, [res.msg].concat(res.parameters.slice(0)));
      if(!/\n$/.test(line)) {
        line += EOL;
      }
      s += line;
  ...
  res = format(opts);
  return util.format.apply(util, [res.msg].concat(res.parameters.slice(0))) + EOL;
}
