...
function find(name, list) {
  ...
  if(ext) {
    ...
    // no longer relevant now that templates may only
    // be javascript, however if this changes (multiple file
    // extensions are allowed) this will enable calling
    // code to determine if a template file reference is ambiguous
    return results;
  }
}
