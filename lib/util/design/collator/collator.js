...
Collator.prototype.getDocs = function(files) {
  ...
  var indices = this.index.docs, attachments = {};
  var docs = [], map = {};
  var i, k, l, v, j, o = {}, id;

  for(i = 0;i < list.length;i++) {
    v = list[i];
    v.path = v.path.replace(rules.docs, '');
    docs.push(v);
  ...
  function getFolderDocumentId(v) {
    return path.basename(path.dirname(v.file));
  }
  ...
  //console.dir('docs output');
  //console.dir(o);
  //console.dir('attachments list');
  //console.dir(attachments);
  return docs.length
    ? {files: docs, documents: o, attachments: attachments} : null;
}
