...
module.exports = function ready(req, next) {
  ...
  this.on('unauth', function(err, info, req, next) {
    // exit with an error if interactivity is off
    // additional conditional prevents the error in test environment
    if(this.interactive === false && this.configure().exit) {
      this.error(err);
    }
  })
