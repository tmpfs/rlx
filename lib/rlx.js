...
Relax.prototype.on = function() {
  ...
  this
    .once('load', function(req) {
      ...
      // in test mode the loaded definition
      // is not cached so this event fires multiple
      // times which causes a listener leak warning
      if(process.env.NODE_ENV === 'test') {
        process.removeAllListeners('exit')
      }

      process.on('exit', function(code) {
        //var conf = scope.configure();
        //var errs = conf.errors || [];
        //if(!conf.interactive && errs.length) {
          // write debug file
          //console.dir(errs);
          debuglog.call(scope, code, req);
        //}
      })
